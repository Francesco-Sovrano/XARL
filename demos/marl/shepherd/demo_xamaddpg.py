...
from xarl.agents.xamaddpg import XAMADDPGTrainer, XAMADDPG_DEFAULT_CONFIG
...
def gen_policy(i):
	return (
		...
		{
			"agent_id": i,
			"use_local_critic": not CENTRALISED_TRAINING,
		}
	)
...
if CENTRALISED_TRAINING:
	...
	policy_graphs[DEFAULT_POLICY_ID] = gen_policy(0)
	def policy_mapping_fn(agent_id):
...
CONFIG.update({
	# === Multi-agent setting ===
	"multiagent": merge_dicts(CONFIG["multiagent"], {
...
train(XAMADDPGTrainer, CONFIG, SELECT_ENV, test_every_n_step=4e7, stop_training_after_n_step=4e7)
