...
SELECT_ENV = "GraphDrive-Easy-Sparse"
...
CONFIG.update({
	...
	"buffer_options": {
		...
		'global_size': 2**14, # Default 50000. Maximum number of batches stored in all clusters (whose number depends on the clustering scheme) of the experience buffer. Every batch has size 'replay_sequence_length' (default is 1).
		...
		'stationarity_window_size_for_real_distribution_matching': None, # Note that this only works when prioritized_drop_probability > 1. stationarity_window_size_for_real_distribution_matching decides whether to use a random number rather than the batch priority during prioritised dropping. If equal to float('inf') then: At time t the probability of any experience being the max experience is 1/t regardless of when the sample was added, guaranteeing that (when prioritized_drop_probability==1) at any given time the sampled experiences will approximately match the distribution of all samples seen so far. If lower than float('inf') and greater than 0, then the stationarity_window_size_for_real_distribution_matching W is used to guarantee that every W training-steps the buffer is emptied from old state transitions.
	...
	"clustering_scheme": None, # Which scheme to use for building clusters. One of the following: "none", "positive_H", "H", "HW", "long_HW", "W", "long_W".
