...
class XAMADDPGPostprocessing:
    """Implements agentwise termination signal and n-step learning."""

    def get_new_actions(self, obs_t):

        return self.sess.run(self.target_act_sampler, {self.new_obs_ph: obs_t})
    
    def compute_td_error(self, obs_t, act_t, rew_t, obs_tp1, done_mask,
                         importance_weights, new_act_t):

        feed_dict = {
            self.rew_ph: rew_t,
            self.done_ph: done_mask,
            self.prio_ph: importance_weights,
        }
        assert len(self.obs_ph_n) == len(obs_t)
        feed_dict.update({k:v for k,v in zip(self.obs_ph_n, obs_t)})
        feed_dict.update({k:v for k,v in zip(self.act_ph_n, act_t)})
        feed_dict.update({k:v for k,v in zip(self.new_obs_ph_n, obs_tp1)})
        feed_dict.update({k:v for k,v in zip(self.new_act_ph_n, new_act_t)})
    ...
    def postprocess_trajectory(self,
        ...
        #  supported now.
        sample_batch[SampleBatch.DONES] = self.get_done_from_info(sample_batch[SampleBatch.INFOS])

        if PRIO_WEIGHTS not in sample_batch:
            sample_batch[PRIO_WEIGHTS] = np.ones_like(sample_batch[SampleBatch.REWARDS])
        if self.config["buffer_options"]["priority_id"] == "td_errors":
            sample_batch["new_actions"] = self.get_new_actions(sample_batch[SampleBatch.CUR_OBS])
            sample_batch["td_errors"] = self.compute_td_error(
                [sample_batch[SampleBatch.CUR_OBS]] + ([] if len(self.obs_ph_n) == 1 else [batch[SampleBatch.CUR_OBS] for policy,batch in other_agent_batches.values()]), 
                [sample_batch[SampleBatch.ACTIONS]] + ([] if len(self.act_ph_n) == 1 else [batch[SampleBatch.ACTIONS] for policy,batch in other_agent_batches.values()]), 
                sample_batch[SampleBatch.REWARDS], 
                [sample_batch[SampleBatch.NEXT_OBS]] + ([] if len(self.new_obs_ph_n) == 1 else [batch[SampleBatch.NEXT_OBS] for policy,batch in other_agent_batches.values()]), 
                sample_batch[SampleBatch.DONES], 
                sample_batch[PRIO_WEIGHTS],
                [sample_batch["new_actions"]] + ([] if len(self.new_act_ph_n) == 1 else [policy.get_new_actions(batch[SampleBatch.CUR_OBS]) for policy,batch in other_agent_batches.values()]), 
            )
            # print(sample_batch["td_errors"])
...
class XAMADDPGTFPolicy(XAMADDPGPostprocessing, TFPolicy):
    def __init__(self, obs_space, act_space, config):
        ...
        self.obs_ph_n = obs_ph_n = _make_ph_n(obs_space_n, SampleBatch.OBS)
        self.act_ph_n = act_ph_n = _make_ph_n(act_space_n, SampleBatch.ACTIONS)
        self.new_obs_ph_n = new_obs_ph_n = _make_ph_n(obs_space_n, SampleBatch.NEXT_OBS)
        self.new_act_ph_n = new_act_ph_n = _make_ph_n(act_space_n, "new_actions")
        self.rew_ph = rew_ph = tf1.placeholder(tf.float32, shape=None, name="rewards_{}".format(agent_id))
        self.done_ph = done_ph = tf1.placeholder(tf.float32, shape=None, name="dones_{}".format(agent_id))
        self.prio_ph = prio_ph = tf1.placeholder(tf.float32, shape=None, name=PRIO_WEIGHTS)
        ...
        # Build critic loss.
        self.td_error = td_error = tf.subtract(
            tf.stop_gradient(
                rew_ph + (1.0 - done_ph) *
                (config["gamma"]**config["n_step"]) * target_critic[:, 0]),
            critic[:, 0])
        prio_ph = tf.cast(prio_ph, tf.float32)
        critic_loss = tf.reduce_mean(prio_ph * (td_error**2))
        ...
        critic, _, _, _ = self._build_critic_network(
            ...
            getattr(tf.nn, config["critic_hidden_activation"]),
            scope="critic")
        actor_loss = -tf.reduce_mean(prio_ph * critic)
        ...
        loss_inputs = _make_loss_inputs(obs_ph_n + act_ph_n + new_obs_ph_n +
                                        new_act_ph_n + [rew_ph, done_ph, prio_ph])
    ...
    def extra_compute_grad_fetches(self):
        return {LEARNER_STATS_KEY: {"td_error": self.td_error}}
