...
from environments.car_controller.multi_agent_graph_drive.ma_graph_drive import MultiAgentGraphDrive
culture_level_list = ["Easy","Medium","Hard"]
for culture_level in culture_level_list:
	...
	register_env(f"MAGraphDrive-{culture_level}-ExplanationEngineering-V1", lambda config: MultiAgentGraphDrive({"reward_fn": 'frequent_reward_explanation_engineering_v1', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-ExplanationEngineering-V2", lambda config: MultiAgentGraphDrive({"reward_fn": 'frequent_reward_explanation_engineering_v2', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-ExplanationEngineering-V3", lambda config: MultiAgentGraphDrive({"reward_fn": 'frequent_reward_explanation_engineering_v3', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-S*J", lambda config: MultiAgentGraphDrive({"reward_fn": 'frequent_reward_step_multiplied_by_junctions', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-FullStep", lambda config: MultiAgentGraphDrive({"reward_fn": 'frequent_reward_full_step', "culture_level": culture_level, **config}))
	...
	register_env(f"MAGraphDrive-{culture_level}-Sparse-ExplanationEngineering-V1", lambda config: MultiAgentGraphDrive({"reward_fn": 'sparse_reward_explanation_engineering_v1', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-Sparse-ExplanationEngineering-V2", lambda config: MultiAgentGraphDrive({"reward_fn": 'sparse_reward_explanation_engineering_v2', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-Sparse-ExplanationEngineering-V3", lambda config: MultiAgentGraphDrive({"reward_fn": 'sparse_reward_explanation_engineering_v3', "culture_level": culture_level, **config}))
	register_env(f"MAGraphDrive-{culture_level}-Sparse-S*J", lambda config: MultiAgentGraphDrive({"reward_fn": 'sparse_reward_step_multiplied_by_junctions', "culture_level": culture_level, **config}))
