...
class PartWorldSomeAgents_GraphDelivery(FullWorldAllAgents_GraphDelivery):
	...
	def get_empty_state_recursively(_obs_space):
		if isinstance(_obs_space, gym.spaces.Dict):
			return {
				k: PartWorldSomeAgents_GraphDelivery.get_empty_state_recursively(v)
				for k,v in _obs_space.spaces.items()
			}
		elif isinstance(_obs_space, gym.spaces.Tuple):
			return list(map(PartWorldSomeAgents_GraphDelivery.get_empty_state_recursively, _obs_space.spaces))
